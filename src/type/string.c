...
size_t deserialize_vector_of_strings(scale_vector *vec, size_t *num_string_elems, const uint8_t *restrict serialized, size_t size) {
  ...
  size_t compact_int_length = read_compact_int_from_data(&compact, serialized, size);
  *num_string_elems = (size_t)decode_compact_to_u64(&compact);
  ...
  for(i = 0; i < *num_string_elems; i++) {
    ...
    size_t str_len = deserialize_string(&scale_string, &serialized[compact_int_length + length], size);
...
extern size_t inline deserialize_string(scale_vector *vec, const uint8_t *restrict serialized, size_t size);
