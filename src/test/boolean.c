...
int run_boolean_test() {
  ...
  assert(decode_boolean(&boolean_decoded) == 0x01);

  assert(out == 0x01);
  assert(decode_boolean(&boolean) == 0x01);
  ...
  assert(encode_boolean_from_hex(&boolean_decoded, hex) == 0);
  assert(decode_boolean(&boolean_decoded) == 0x00);

  printf("\t\tBoolean.<false> is: <%s>\n", hex);
  free(hex);
  assert(out == 0x00);
  assert(decode_boolean(&boolean) == 0x00);
  return 0;
}
